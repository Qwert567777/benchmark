...
#            "filebench_fileserver",
    ...
    def mount_NOVA(self, media, fs, mnt_path):
        ...
        p = self.exec_cmd(' '.join(["sudo mount -t", fs, "-o init,bdev=/dev/nvme0n1",
                                    dev_path, mnt_path]),
                          self.dev_null)
        if p.returncode is not 0:
            return False
...
if __name__ == "__main__":
    ...
    if any(["/dev/pmem" not in dev for dev in [Runner.HDDDEV, Runner.SSDDEV, Runner.NVMMDEV]]):
        confirm_media_path()
